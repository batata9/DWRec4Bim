...
public class DAOAluguel extends DAOGenerico<Aluguel> {
    ...
    public List<Aluguel> listByNome(String nome) {
        return em.createQuery("SELECT e FROM Aluguel e WHERE e.clienteLogin LIKE :nome").setParameter("nome", "%" + nome + "%").getResultList();
    }
    ...
    public List<Aluguel> listInOrderNome() {
        return em.createQuery("SELECT e FROM Aluguel e ORDER BY e.clienteLogin").getResultList();
    }
    ...
    public List<String> listInOrderNomeStrings(String qualOrdem) {
        ...
        for (int i = 0; i < lf.size(); i++) {
            ls.add(lf.get(i).getIdAluguel() + "-" + lf.get(i).getClienteIdCliente());
        }
        return ls;
    }
public static void main(String[] args) {
        ...
        for (Aluguel aluguel : listaAluguel) {
            System.out.println(aluguel.getIdAluguel() + "-" + aluguel.getClienteIdCliente());
        }
    }}
